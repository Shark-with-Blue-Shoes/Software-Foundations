...
Check perm3_trans [1; 2; 3] [2; 1; 3] [3; 2; 1].
Check Perm3 [1; 2; 3] [2; 1; 3].
...
Theorem ev_double : forall n,
  ev (double n).
Proof.
intros n. induction n. simpl. apply ev_0. simpl. apply ev_SS. apply IHn.
Qed.
...
Proof.
intros n m H. destruct H. left. reflexivity. right. exists m. split.
reflexivity. apply H.
Qed.
...
Proof.
  intros. induction H0. apply H. simpl in H. inversion H. apply IHev. apply H2.
Qed.
...
Lemma two_plus_n : forall n, S (S n) = 2 + n.
...
Theorem le_plus_l : forall a b,
  a <= a + b.
Proof.
  intros a b. induction b. rewrite add_0_r. reflexivity. 
  rewrite add_comm. simpl. apply (le_S a (b+a)). rewrite add_comm.
  apply IHb.
Qed.
...
Theorem plus_le : forall n1 n2 m,
  ...
  n1 <= m /\ n2 <= m.
Proof.
  intros n1 n2 m H. induction H. split.
  - apply le_plus_l. 
  - rewrite add_comm. apply le_plus_l.
  - destruct IHle. split. apply (le_S n1 m). apply H0.
    apply (le_S n2 m). apply H1.
Qed.
...
Proof.
intros.
induction H. reflexivity. assert(H1: p + S m = S m + p). 
- rewrite add_comm. reflexivity.
- rewrite H1. simpl. apply (le_S (p+n) (m + p)). assert (m+p = p + m).
apply add_comm. rewrite H0. apply IHle.
Qed.
...
Proof.
  ...
  rewrite add_comm. simpl. rewrite add_comm. apply (le_S n (m + p)). apply IHp.
Qed.
...
Proof.
  unfold ge. unfold Reflexive. intros. apply le_n.
Qed.
...
Theorem n_lt_m__n_le_m : forall n m,
  ...
  n <= m.
Proof.
intros. induction H. apply le_S. reflexivity. apply le_S. apply IHle.
Qed.
...
Proof.
  ...
  - rewrite <- Hm in H. apply Sn_le_Sm__n_le_m in H.
    ...
    split. apply n_le_m__Sn_le_Sm. apply H1.
...
Lemma MUnion' : forall T (s : list T) (re1 re2 : reg_exp T),
  ...
  s =~ Union re1 re2.
Proof.
intros T s re1 re2 H. destruct H. apply MUnionL.
- apply H.
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp T),
  ...
  fold app ss [] =~ Star re.
Proof.
intros. induction ss. simpl. apply MStar0. simpl. apply MStarApp. 
Search (_ =~ re). apply H. simpl. left. reflexivity.
